...
function check_git_access() {
    # Attempt to SSH into the server with a timeout of 10 seconds
		ssh -q -o ConnectTimeout=120 ${GIT_SERVER##*/} 'exit' 2>/dev/null
		if [ $? -eq 0 ]; then
...
function check_airtable_api_key() {
...
function fetch_airtable_record_id_by_hostname() {
...
function update_ssh_key_in_airtable() {
...
function clone_repository() {
    ...
    local branch=$2
		while [ 1 -eq 1 ]; do
			...
			git clone --single-branch --branch ${branch} ${GIT_SERVER}/${repo_name} 2>&1 | tee /tmp/git_output.txt
			if [ $? -ne 0 ]; then
				PERM_DENIED=$(sed -n '/Permissions denied/=' /tmp/git_output.txt)
				if [ "${PERM_DENIED}" ]; then
					cecho "YELLOW" "Git clone of $repo_name failed. Retrying..."
		      sleep 60
				else
					cecho "RED" "Git clone of $repo_name error. Exiting..."
					exit 1;	
				fi
			else	# git succeeded
				break;	# stop the loop
...
function create_systemd_service() {
    ...
    cat >/etc/systemd/system/fleet-flows-js.service <<EOL
[Unit]
...
WorkingDirectory=$HOME/fleet-flows-js)
Restart=always
User=$(whoami)
...
function create_auto_update_job() {
...
function create_fleet_flow_js_listener_service() {

    sudo cat >/etc/systemd/system/fleet-flows-js-listener.service <<EOL
[Unit]
...
N_CMD=$(which n 2>/dev/null)
if [ "${N_CMD}" ]; then
	debug_echo "DEBUG" "installed ${N_CMD}"
else
...
NRED_CMD=$(which node-red 2>/dev/null)
if [ "${NRED_CMD}" ]; then
	debug_echo "DEBUG" "installed ${NRED_CMD}"
else
